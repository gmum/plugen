...
class CelebA(Dataset):
    def __init__(self, path_zip, path_txt, image_size, crop_size=178, fully_supervised=False, labels_per_image=5):
        ...
        with open(path_txt, "r") as f:
            ...
            self.mask = torch.zeros((202599, self.label_size))
            for l in f.readlines():
                name, *labels = re.split(r"\s+", l.strip())
                name = re.search(r'[0-9]+', name).group(0)
                name = int(name)-1
                labels = [1. if i == "1" else -1. for i in labels]
                # labels = [labels[20], labels[31], labels[15]]
                self.labels[name] = torch.tensor(labels)
                mask_indices = rng.choice(
                        self.label_size, size=labels_per_image, replace=False)
                self.mask[name][mask_indices] = 1.

        self.mask = self.mask.bool()
        if fully_supervised:
            self.mask = torch.ones_like(self.mask).bool()

        print("loading", path_zip)
        self.zip_file = zipfile.ZipFile(path_zip, "r")

    def __getitem__(self, index):
        image = self._read_zipped_image(self.zip_file, abs(index), index < 0)
        mask = self.mask[index]
        return image, self.labels[index], mask
